...
// Renders editorial-style "product photos" entirely in SVG so each piece is unique.
...
const MarblePiece = ({
  ...
  // Furniture silhouettes — each returns paths inside a 1000x1000 box, vertically centered around y=500
  const silhouettes = {
    // 1. Console table — long top, two slab legs
    console: (
      <g>
    ...
    // 2. Round coffee table — disc top, columnar base
    coffee: (
      <g>
    ...
    // 3. Plinth — solid rectangular block
    plinth: (
      <g>
    ...
    // 4. Round side table — small disc with single carved column
    side: (
      <g>
    ...
    // 5. Vase stand / pillar — fluted column
    pillar: (
      <g>
    ...
    // 6. Dining table — wide oval top, sculptural pedestal base
    dining: (
      <g>
    ...
    // 8. Sculptural stool / orb on cylinder
    orb: (
      <g>
  ...
  return (
    ...
    >
      <defs>
        ...
        {/* marble base gradient — light variation */}
        ...
        {/* marble vein filter — turbulence + displacement gives organic veining */}
      ...
      <g filter={`url(#${sId})`}>
        <g clipPath={`url(#${clipId})`}>
          ...
          {/* veins — multiple turbulent strokes inside the clip */}
          <MarbleVeins seed={seed} color={veinColor} highlight={highlight} />
        </g>
        {/* subtle edge highlight on the piece */}
        <g transform={`rotate(${rotate} 500 540)`} fill="none" stroke={highlight} strokeOpacity="0.35" strokeWidth="1.2">
          {silhouette}
        </g>
      </g>
...
// Marble veining — layered organic strokes
const MarbleVeins = ({ seed, color, highlight }) => {
  // generate pseudo-random vein paths from seed
  const rand = (() => {
    let s = seed * 1000 + 7;
    return () => {
  ...
  // a couple of bright highlight veins
  for (let i = 0; i < 3; i++) {
...
window.MarblePiece = MarblePiece;
